...
namespace ConsoleApp1
{
    public class Router
    {
        ...
        {
            string path = PathUtility.ConfigPath() + "/http-listener-config.json";
        ...
        public void Init(List<string> urlBases)
        {
            ...
            Debug.Log($"Create routing local paths with {urlBases.Count} url bases  : ", ConsoleColor.White,ConsoleColor.Blue);
            ...
            foreach (var assembly in assemblies)
            {
                foreach (Type t in assembly.GetTypes())
                {
                    if (t.GetCustomAttributes(typeof(MyApiControllerAttribute), true).Length > 0)
                    {
                        ...
                        ControllersContainer.AddItem(t,instance);
                        ...
                        foreach (MethodInfo methodInfo in methods)
                        {
                            ...
                            foreach (var attr in attrs)
                            {
                                if (attr is MyRestRouteAttribute)
                                {
                                    ...
                                    httpController.instance = instance;
